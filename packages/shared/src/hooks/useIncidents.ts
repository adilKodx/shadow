...
export function useIncidents() {
  ...
  // ── Realtime: keep `incidents` and `incident_responders` in sync without
  // polling or waiting for the next manual mutation. The LiveMap badge count
  // and drawer list update the moment another user (or server) changes a row.
  useEffect(() => {
    if (!tenant?.id) return;
    const channel = supabase
      .channel(`incidents:${tenant.id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'incidents', filter: `tenant_id=eq.${tenant.id}` },
        () => { fetchIncidents(); },
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'incident_responders' },
        () => { fetchResponders(); },
      )
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [tenant?.id, fetchIncidents, fetchResponders]);
