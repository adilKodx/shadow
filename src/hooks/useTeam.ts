...
export function useTeam() {
  ...
  // Manually add a member by email + password + role (creates auth user + tenant_members record)
  const addMember = useCallback(async (email: string, password: string, displayName: string, role: string) => {
    if (!tenant || !user) return { error: 'Not authenticated' };
    if (!canInvite) return { error: `Member limit reached (${memberLimit}). Upgrade your plan.` };
    ...
    if (!newUid) return { error: 'Failed to create user' };

    // 2. Insert tenant_members record
    ...
    await fetchMembers();
    return { error: null };
  }, [tenant, user, canInvite, memberLimit, fetchMembers]);
  ...
  return {
    ...
    fetchMembers, updateMember, removeMember, addMember,
